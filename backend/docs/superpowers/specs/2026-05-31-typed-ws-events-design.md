...
**无 schema 变化。** 复用现有 `suggestions` 表与 `SuggestionRepository` 的全部方法（已具备完整状态机）：
- `InsightReady` → `insert_direct(...)`
- `AnalysisProposed` → `upsert_pending(...)`
- `confirm_analysis` 路径：`mark_running(...)` → `upsert_ready(...)`
- `AnalysisDismissed.reason="dismissed"` → `mark_dismissed(...)`
- `AnalysisDismissed.reason="expired"` → `mark_expired(...)`
...
| `backend/src/repositories/suggestions.py` | 无需改动（现有状态机已覆盖） | 0 |
