"""
端到端验证脚本 v2：一次性 diarization（ClusterBackend）

用法:
    # 完整流程（有律师声纹）
    uv run python scripts/validate_pipeline_v2.py \
...
async def run(conv_path: str, lawyer_path: str | None, session_id: str) -> None:
    ...
    # ── Step 3: 运行 pipeline v2 ─────────────────────────────────────────────
    print("\n[3/3] 运行 pipeline v2（一次性 diarization）...")
    t0 = time.time()
    ...
    for i, r in enumerate(results, 1):
        label = {"律师": "🧑‍⚖️ 律师", "客户": "👤 客户", "未知": "❓ 未知"}.get(r.speaker, r.speaker)
        print(f"  [{i:02d}] {label}：{r.text}")
    print(f"{'─'*60}\n")


def main():
    parser = argparse.ArgumentParser(description="funASR pipeline v2 端到端验证")
