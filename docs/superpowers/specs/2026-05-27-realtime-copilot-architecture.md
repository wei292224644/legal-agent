...
4. **窗口由对话节奏驱动,不是固定时间**——VAD 沉默 ≥400ms 关闭 utterance,累计说话 ≥8s 进入软切待机(下一个 ≥150ms 微停顿处切)。
...
|------|------|------|------|----------|
| Track 1 STT | ~700ms(soft_cap 立切)~ ~900ms(400ms VAD + ~500ms 处理) | utterance 关闭 | `transcript.final`(整句) | 字幕按 utterance 跳出,不是 token 流 |
...
| 触发 | 行为 |
|------|------|
| `fsmn-vad` 检测沉默 ≥ 400ms | 正常关闭(主路径,覆盖典型对话节奏的 ≥85% 场景) |
| 累计说话时长 ≥ 8s | 进入"软切待机":从**下一次**任意 ≥ 150ms 微停顿处切,**立即** yield(不等 silence 满 VAD 阈值,切分点已由能量微停顿确定) |

8s 的 cap 是律师视觉刷新周期的舒适上限(超此值副屏看起来"卡死")。**不硬切**——硬切在数字 / 法条编号中间("两万五" → "两万" + "五")会摧毁下游意图识别。脚本里 L19 / L65 那种 17-25s 的长独白靠软切处理。

> 常量值经 sprint 1 真实数据校准:`VAD_SILENCE_MS=400`(原 1500,1.5s 让用户感知延迟过高),`MICROPAUSE_MS=150`(原 300,真实对话节奏紧凑)。详见 `2026-05-27-sprint1-stt-handoff.md`。
